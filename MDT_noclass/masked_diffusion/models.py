...
#De-scaling an input of [-1,1] to [0,255]
def Orig_Img(x):
    return ((x+1) * 127.5).clamp(0, 255)
...
class ImageEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        # Additional linear layer (unfrozen) to get to the desired hidden_size
    ...
    def forward(self, x):
        ...
        # Apply preprocessing to each image using the defined transforms
        processed_img = self.preprocess(x)
    
        # Extract features with ResNet18
        features = self.resnet18(processed_img)
        # Flatten the features
        features = features.view(features.size(0), -1)
        # Pass the flattened features through the additional linear layer
...
class MDT(nn.Module):
    ...
    def __init__(
        ...
        self.img_embedder = ImageEmbedder(hidden_size)
        ...
        print("mask ratio:", self.mask_ratio,
              ...
              "class conditioning:", self.class_cond)
        self.initialize_weights(class_cond)

    def initialize_weights(self, class_cond):
        ...
        # Original MDT model is a class conditional model. To train this version with image embeddings
        # instead of label embeddings, class_cond is set to False and the model is further trained using
        # the checkpoint of the original model. All parameters are frozen, except for the last layer
        # of the image embedder.

        if not class_cond:

            # Initialize image embedding FC layer:
            nn.init.xavier_uniform_(self.img_embedder.fc.weight, gain=1)
            if hasattr(self.img_embedder.fc, 'bias') and self.img_embedder.fc.bias is not None:
                nn.init.constant_(self.img_embedder.fc.bias, 0)

            # Freeze all parameters initially
            for param in self.parameters():
                param.requires_grad = False

            # Unfreeze the parameters of the FC layer in y_embedder
            for param in self.img_embedder.fc.parameters():
    ...
    def forward(self, x, t, original_images, y, class_cond, enable_mask=False):
        """
        Forward pass of MDT.
        x: (N, C, H, W) tensor of spatial inputs (latent representations of images)
        t: (N,) tensor of diffusion timesteps
        original_images: (N, c, h, w) tensor of spatial inputs (original input images with original size)
        y: (N,) tensor of class labels
        class_cond: Use label embedding if True, image embedding if False
        ...
        if class_cond:
            ...
            if original_images is None:
                raise SystemExit("Original images not found.")
            y = self.img_embedder(original_images)
    ...
    def forward_with_cfg(self, x, t, original_images, y, class_cond, cfg_scale=None, diffusion_steps=1000, scale_pow=4.0):
        """
        ...
        if cfg_scale is not None:
            ...
            model_out = self.forward(combined, t, y, original_images, class_cond)
            ...
            if original_images is None:
                raise SystemExit("Original images not found.")
            model_out = self.forward(x, t, original_images, y, class_cond)
            eps, rest = model_out[:, :3], model_out[:, 3:]
            return torch.cat([eps, rest], dim=1)
