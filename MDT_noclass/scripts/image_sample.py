...
def main():
    ...
    model = models_mdt.__dict__[args.model](input_size=latent_size, decode_layer=args.decode_layer, class_cond=args.class_cond)
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        model_kwargs = {}
        # Added class_cond to kwargs
        model_kwargs["class_cond"] = args.class_cond
        # Added an encoding step to original size inputs and original images to kwargs
        orig_img = th.randn(args.batch_size, 3, image_size, image_size, device=dist_util.dev())
        z = get_first_stage_encoding(orig_img)
        # Added original images to kwargs
        model_kwargs["original_images"] = orig_img
        ...
        if args.cfg_cond:         
            ...
            # Added if statement for class conditioning           
            if args.class_cond:
                classes = th.randint(
        ...
        if model_kwargs["original_images"] is None:
    ...
    if dist.get_rank() == 0:
        ...
        out_path = os.path.join(logger.get_dir(), f"samples_origwc_{shape_str}.npz")
